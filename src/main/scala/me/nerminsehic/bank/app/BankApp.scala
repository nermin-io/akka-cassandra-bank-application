package me.nerminsehic.bank.app

import akka.actor.typed.{ActorRef, ActorSystem, Behavior}
import akka.actor.typed.scaladsl.Behaviors
import me.nerminsehic.bank.actors.Bank
import me.nerminsehic.bank.actors.PersistentBankAccount.Command
import akka.actor.typed.scaladsl.AskPattern._
import akka.http.scaladsl.Http
import akka.util.Timeout
import me.nerminsehic.bank.http.BankRouter
  ...
  private def startHttpServer(bank: ActorRef[Command])(implicit system: ActorSystem[_]): Unit = {
    ...
    httpBindingFuture.onComplete {
      case Success(binding) =>
        ...
        system.log.info(s"Server started on http://${address.getHostString}:${address.getPort}")
